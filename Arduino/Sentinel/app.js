...
function renderForensicAnalysis(records, radio) {
  ...
  $("#payloadHistory").innerHTML = `<h4>Distinct payloads (${payloads.size})</h4>${[...payloads.values()].sort((a, b) => b.last - a.last).slice(0, 12).map((payload, index) =>
...
function renderSessionComparison(results) {
  ...
  if (results.length === 1) {
    ...
    $("#sessionAnalysis").innerHTML = `<h3>Session review</h3>
      ...
      <h3>Timeline preview</h3><article class="insight"><span>${timeline(result).map(event => `${escapeHtml(formatDate(event.at))} — ${escapeHtml(event.label)}`).join("<br>") || "No events"
      }</span></article>`;
    return;
  }
  ...
  $("#sessionAnalysis").innerHTML = `
    ...
    ${results.map(result => `<article class="insight"><strong>${escapeHtml(result.id)}</strong><span>${timeline(result).map(event => `${escapeHtml(formatDate(event.at))} — ${escapeHtml(event.label)}`).join("<br>") || "No events"
    }</span></article>`).join("")}`;
}
